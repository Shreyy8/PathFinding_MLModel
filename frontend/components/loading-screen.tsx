...
export function LoadingScreen({ onLoadingComplete, minimumDuration = 3500 }: LoadingScreenProps) {
  ...
  // Generate road patterns for each tile (client-only, so no hydration issues)
  ...
  // Randomize tile reveal order (client-only, so no hydration issues)
  ...
  // Pre-calculate tile brightness for consistency (client-only, so no hydration issues)
